---
type: todo
created: 2026-01-18
modified: 2026-01-18 22:05
tags:
categories:
projects:
due date:
priority:
complete: false
permalink:
---
```habittracker
{
  "path": "Habits",
  "daysToShow": 30
}
```
👆[[2026 weekly resolutions extended resolution list]]

**Replace Thesis**

- [ ] Buy Stocks

DGIBG
- [ ] Read over manuscript
	- [ ] Make updates based on original notes - use ghostty panels
PAIRENT
- [ ] Record my audio
- [ ] Test with already recorded audio
- [ ] Allow to record audio after start using
	- [ ] How to add/update?

SIDE PROJECT
- [ ] Check strava project idea
- [x] CAF Email follow up

ADMIN
- [ ] Donna CPAM
- [ ] Go through black book

- [x] Book fondo
- [ ] Book bikes
- [x] Book Corsica

### Put out 🔥
- [x] Send Cendri dataframe
- [x] Apple edits
- [x] Guarantme
- [x] Monaco medical form, SAS
- [ ] New credit card
- [x] CPAM application (takes 6 weeks)
	- [x] 3 last quittances loyer/electric bills
	- [x] copy of carte de sejour
	- [x] [s1106](https://www.ameli.fr/sites/default/files/formulaires/168/s1106_-_demande_puma_-_assurance_maladie.pdf)
- [x] Fill out life insurance document
- [x] Reply to apartment broker
- [x] ETD changes/update
- [ ] Donna job options
- [x] CPAM
- [ ] Check due applications
	- [ ] Update resume
	- [ ] Apply Nabla
- [x] Decide on French class
- [x] Donate Estrosi
- [ ] Relist phone
- [ ] Relist wetsuit
- [x] Monaco race medical certificate
- [x] Check Donna voting
- [x] Check Donna Amos account
- [x] Call re: CAF missing documents?
### Project
- [ ] Start class
- [ ] Website
- [ ] Daily morning podcast setup
- [ ] [[pairent]] setup
- [ ] Order hardware for [[b(ready)]]
### Future
- [ ] Fill out [[apply to future options opportunities]]
### Admin
- [ ] US Taxes
- [ ] Amos disability account
- [ ] Cleaner/sitter set up
- [ ] Amos genetic screening

### Personal
- [ ] Blood test (iron, testosterone)
- [ ] Dentist appointment
- [ ] Quote for invisiline

### Weekly on Sunday
- [ ] Datenight setup
- [ ] France report for mom
- [ ] Weekly lunch plan